...
2. Japanese Wikipedia Entity Vectors
...
- **Too small vocabulary size in embeddings**  
  → Will try the full dataset, but it might take too long.

- **Method of constructing input vectors from the two words may be inappropriate**  
  → Currently, I simply concatenate the two 200-dimensional vectors (200 + 200 = 400 dimensions). I should consider improving this approach.    
?????
